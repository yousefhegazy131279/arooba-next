...
export async function GET() {
  try {
    ...
    // 2. البحث عن توكن المصادقة – عادة ما يكون اسمه يبدأ بـ 'sb-' وينتهي بـ '-auth-token'
    ...
    // إذا لم نجد التوكن بالشكل المذكور، نحاول البحث عن أي cookie قيمته طويلة تحتوي على نقطتين
    // لأنه قد يكون مخزناً بشكل مختلف
    if (!accessToken) {
      for (const cookie of allCookies) {
    ...
    // 5. استخدام service role لجلب المستخدمين
